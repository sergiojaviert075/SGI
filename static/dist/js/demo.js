...
(function ($) {
  ...
  function createSkinBlock(colors, callback, noneSelected) {
    var $block = $('<select />', {
      class: noneSelected ? 'custom-select mb-3 border-0' : 'custom-select mb-3 text-light border-0 ' + colors[0].replace(/accent-|rr-/, 'bg-')
    })
    ...
    colors.forEach(function (color) {
      var $color = $('<option />', {
        class: (typeof color === 'object' ? color.join(' ') : color).replace('rr-', 'bg-').replace('accent-', 'bg-'),
        text: capitalizeFirstLetter((typeof color === 'object' ? color.join(' ') : color).replace(/rr-|accent-|bg-/, '').replace('-', ' '))
      })
  ...
  var $header_fixed_checkbox = $('<input />', {
    type: 'checkbox',
    value: 1,
    checked: $('body').hasClass('layout-rr-fixed'),
    class: 'mr-1'
  }).on('click', function () {
    if ($(this).is(':checked')) {
      $('body').addClass('layout-rr-fixed')
    } else {
      $('body').removeClass('layout-rr-fixed')
    }
  })
  ...
  var rr_all_colors = rr_dark_skins.concat(rr_light_skins)
  var $rr_variants_colors = createSkinBlock(rr_all_colors, function () {
    var color = $(this).find('option:selected').attr('class')
    var $main_header = $('.main-header')
    $main_header.removeClass('rr-dark').removeClass('rr-light')
    rr_all_colors.forEach(function (color) {
      $main_header.removeClass(color)
    })

    $(this).removeClass().addClass('custom-select mb-3 text-light border-0 ')

    if (rr_dark_skins.indexOf(color) > -1) {
      $main_header.addClass('rr-dark')
      $(this).addClass(color).addClass('text-light')
    } else {
      $main_header.addClass('rr-light')
      $(this).addClass(color)
    }
  ...
  var active_rr_color = null
  $('.main-header')[0].classList.forEach(function (className) {
    if (rr_all_colors.indexOf(className) > -1 && active_rr_color === null) {
      active_rr_color = className.replace('rr-', 'bg-')
    }
  })

  $rr_variants_colors.find('option.' + active_rr_color).prop('selected', true)
  $rr_variants_colors.removeClass().addClass('custom-select mb-3 text-light border-0 ').addClass(active_rr_color)

  $rr_variants.append($rr_variants_colors)

  $container.append($rr_variants)
  ...
  $('body')[0].classList.forEach(function (className) {
    if (accent_colors.indexOf(className) > -1 && active_accent_color === null) {
      active_accent_color = className.replace('rr-', 'bg-')
    }
  })
  ...
  var $brand_variants = createSkinBlock(logo_skins, function () {
    ...
    if (color === 'rr-light' || color === 'rr-white') {
    ...
    if (color) {
      $(this).removeClass().addClass('custom-select mb-3 border-0').addClass(color).addClass(color !== 'rr-light' && color !== 'rr-white' ? 'text-light' : '')
    } else {
  ...
  $('.brand-link')[0].classList.forEach(function (className) {
    if (logo_skins.indexOf(className) > -1 && active_brand_color === null) {
      active_brand_color = className.replace('rr-', 'bg-')
    }
  })
